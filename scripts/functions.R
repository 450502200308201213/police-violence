...
fitmod <- function(data=dta,y="rate_adj", vb = "tmean+prec",  fe="fipsmo + stateyear", cl="fips", inter="no", intvar="", name="") {
  ...
  # Draw a graph of the relationship between temperature and the adjusted ratio
  ggplot(data, aes(x=tmean, y=rate_adj)) +
    geom_point() +
    labs(title=paste(name, "vs Temperature"), x="Temperature", y="Adjusted Rate") +
    theme_minimal()
...
runreg <- function (lag=0,lead=0,y="rate_adj",temp="tmean",prec="prec",data=dta,weights=dta$popw,fe="fipsmo + stateyear",cl="fips") {
  ...
  # Draw an error bar chart of regression coefficients
  r_data <- rbind(r1, r2)
  ggplot(r_data, aes(x=group, y=est, ymin=est-se, ymax=est+se)) +
    geom_pointrange() +
    labs(title="Regression Coefficients with Error Bars", x="Group", y="Coefficient") +
    theme_minimal()
